...
const setup = (errorMessage) => {
  const testID = 'payment.vendor.google-pay-button';

  const Component = <GooglePayButton style={{}} componentId="test" />;
    ...
    describe('When onPress is fired', () => {
      test('Then it must log an error', async () => {
        ...
        expect(global.Logger.error).toHaveBeenCalledWith(
          '[PAYMENT-METHODS] handleGooglePay is called without correct arguments: undefined, undefined, undefined, undefined',
        );
      });
    });
  });
